...
| `POST /rescan/<machine>`
| Re-walk a registered machine's roots to refresh its tracked repos. Needs no token (reuses
  the SSH access authorized at enrollment). Returns 404 if the machine has no recorded roots.
|===
