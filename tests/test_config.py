...
def _t(name, machine):
    ...
    return Target(name=name, machine=machine, repo="/r", ssh_host=None)


def test_display_name_strips_machine_prefix():
    t = _t("winbox-example-repo", "winbox")
    assert t.display_name == "example-repo"


def test_display_name_no_machine_prefix():
    t = _t("diff-lab", "local")
    assert t.display_name == "diff-lab"


def test_display_name_partial_prefix_not_stripped():
    # machine is "dev" but name starts with "devbox-" — should not strip
    t = _t("devbox-Repo", "dev")
    assert t.display_name == "devbox-Repo"


def test_display_name_collision_suffix_kept():
    t = _t("dev-MyRepo-2", "dev")
    assert t.display_name == "MyRepo-2"


def test_display_name_single_segment_name():
    # name has no hyphen at all — returned as-is
    t = _t("myrepo", "local")
    assert t.display_name == "myrepo"


def test_display_name_machine_equals_name_prefix_only():
    # machine "foo" and name "foo-bar" — strip works
    t = _t("foo-bar", "foo")
    assert t.display_name == "bar"
