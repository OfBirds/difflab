...
def test_pubkey_returns_gated_line(enroll_client):
    resp = enroll_client.get("/pubkey")
    assert resp.status_code == 200
    assert resp.content_type.startswith("text/plain")
    line = resp.data.decode().strip()
    # Must have forced-command prefix pointing at the POSIX gate script
    assert line.startswith('command="')
    assert 'git-gate.sh",' in line
    # Restriction options must follow the command= option
...
def test_pubkey_posix_and_default_are_identical(enroll_client):
    default = enroll_client.get("/pubkey").data.decode().strip()
    explicit = enroll_client.get("/pubkey?os=posix").data.decode().strip()
    assert default == explicit


def test_pubkey_windows_form(enroll_client):
    resp = enroll_client.get("/pubkey?os=windows")
    assert resp.status_code == 200
    line = resp.data.decode().strip()
    # Must reference the PowerShell gate invocation
    assert line.startswith('command="')
    assert 'git-gate.ps1",' in line
    assert 'powershell' in line.lower()
    assert 'no-pty' in line
    assert 'ssh-ed25519' in line
    assert line.endswith("difflab")


def test_pubkey_unknown_os_falls_back_to_posix(enroll_client):
    line = enroll_client.get("/pubkey?os=freebsd").data.decode().strip()
    assert 'git-gate.sh",' in line
...
# ──────────────────────────────────────────────
# 19. Re-enroll same machine+repo — no suffix
# ──────────────────────────────────────────────

def test_register_reenroll_same_repo_updates_in_place(data_dir, tmp_path, monkeypatch):
    """Re-enrolling same machine+repo updates the entry in place; no -2 suffix."""
    cfg = {"targets": [{"name": "local-repo", "machine": "local", "repo": "/srv/repos/diff-lab"}]}
    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump(cfg), encoding="utf-8")

    # Pre-seed registry with existing target for machine "dev"
    reg = {
        "targets": [{
            "name": "dev-ThoseDaysApp",
            "machine": "dev",
            "repo": "/srv/repos/ThoseDaysApp",
            "ssh_host": "old@dev.example.com",
            "port": 22,
            "shell": "posix",
        }]
    }
    (data_dir / "registry.yaml").write_text(yaml.dump(reg), encoding="utf-8")
    ...
    body = {**VALID_BODY, "name": "dev"}
    monkeypatch.setattr(enroll_mod, "_run_ssh",
                        lambda argv: ("/srv/repos/ThoseDaysApp/.git\n", "", 0))
    ...
    assert resp.status_code == 200
    data = resp.get_json()
    assert "dev-ThoseDaysApp" in data["targets"]
    assert not any(t.endswith("-2") for t in data["targets"])

    reg_after = yaml.safe_load((data_dir / "registry.yaml").read_text(encoding="utf-8"))
    dev_entries = [t for t in reg_after["targets"] if "ThoseDaysApp" in t.get("repo", "")]
    assert len(dev_entries) == 1
    assert dev_entries[0]["name"] == "dev-ThoseDaysApp"
    assert dev_entries[0]["ssh_host"] == "alice@192.0.2.10"


# ──────────────────────────────────────────────
# 23. /register calls status_service.invalidate
# ──────────────────────────────────────────────

def test_register_calls_status_service_invalidate(enroll_app, monkeypatch):
    """/register must invalidate the status cache for the enrolled machine."""
